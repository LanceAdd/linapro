...
func (*manifestHostService) Get(path string) ([]byte, bool, error) {
	request := &HostServiceManifestGetRequest{Path: path}
	payload, err := invokeHostService(
		HostServiceManifest,
		HostServiceMethodManifestGet,
		path,
		"",
		MarshalHostServiceManifestGetRequest(request),
	)
	if err != nil {
		return nil, false, err
	}
	response, err := UnmarshalHostServiceManifestGetResponse(payload)
	if err != nil {
		return nil, false, err
	}
