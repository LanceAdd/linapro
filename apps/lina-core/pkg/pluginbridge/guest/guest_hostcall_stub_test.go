...
func TestHostCallStubsReturnUnavailable(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "http", run: func() error {
			_, err := HTTP().Request("https://example.com", &HostServiceNetworkRequest{})
			return err
		}},
		{name: "data", run: func() error {
			_, err := Data().List("sys_demo", nil, 1, 10)
			return err
		}},
		...
		{name: "notify", run: func() error {
			_, err := Notify().Send("demo", &HostServiceNotifySendRequest{})
			return err
		}},
		{name: "cron", run: func() error {
			return Cron().Register(&CronContract{})
		}},
