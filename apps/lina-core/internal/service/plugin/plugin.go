...
type (
	...
	// ConsumerSurfaceSnapshot describes the host-governed plugin consumer
	// surface projection across source plugins.
	ConsumerSurfaceSnapshot struct {
		// Plugins contains one plugin consumer surface projection per source plugin with C-side capabilities.
		Plugins []*ConsumerSurfacePluginSnapshot
	}

	// ConsumerSurfacePluginSnapshot describes one source plugin's consumer-facing surface projection.
	ConsumerSurfacePluginSnapshot struct {
		...
		// ConsumerAPIRouteCount is the number of C-side API route bindings.
		ConsumerAPIRouteCount int
		// ConsumerAPIRoutes contains stable metadata for C-side API route bindings.
		ConsumerAPIRoutes []*ConsumerSurfaceRouteSnapshot
		// ConsumerFrontend contains the optional C-side frontend mount projection.
		ConsumerFrontend *ConsumerSurfaceFrontendSnapshot
	}

	// ConsumerSurfaceRouteSnapshot describes one captured C-side source-plugin route.
	ConsumerSurfaceRouteSnapshot struct {
		// Method is the normalized HTTP method.
		Method string
		// Path is the public route path registered on the host.
		Path string
		// Documentable reports whether the route can be projected into OpenAPI.
		Documentable bool
	}

	// ConsumerSurfaceFrontendSnapshot describes one source-plugin C-side frontend mount.
...
// ConsumerSurfaceGovernanceService defines host-governed plugin consumer surface projections.
type ConsumerSurfaceGovernanceService interface {
	// BuildConsumerSurfaceSnapshot builds an on-demand host governance snapshot
	// for source-plugin consumer API routes, frontend mounts, enablement,
	// version, and tenant governance declarations.
