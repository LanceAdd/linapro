// This file builds host-governed plugin consumer surface projections by
// combining source-plugin route bindings, manifest declarations, frontend
// resource indexes, and current enablement state.
...
// for source-plugin consumer API routes, frontend mounts, enablement, version,
// and tenant governance declarations.
...
func buildConsumerSurfaceSnapshot(
	...
	for _, binding := range bindings {
		if binding.Surface != pluginhost.SurfaceConsumer {
			continue
		}
		pluginID := strings.TrimSpace(binding.PluginID)
		if pluginID == "" {
			pluginID = pluginhost.ConsumerPluginIDFromPath(binding.Path)
		}
		if pluginID == "" {
			continue
		}
		item := pluginSnapshots[pluginID]
		if item == nil {
			item = &ConsumerSurfacePluginSnapshot{PluginID: pluginID}
			pluginSnapshots[pluginID] = item
		}
		item.ConsumerAPIRoutes = append(item.ConsumerAPIRoutes, &ConsumerSurfaceRouteSnapshot{
	...
	for _, item := range pluginSnapshots {
		...
		sortConsumerSurfaceRoutes(item.ConsumerAPIRoutes)
		item.ConsumerAPIRouteCount = len(item.ConsumerAPIRoutes)
...
func consumerSurfacePluginHasCapability(item *ConsumerSurfacePluginSnapshot) bool {
	return item != nil && (len(item.ConsumerAPIRoutes) > 0 || item.ConsumerFrontend != nil)
}

// sortConsumerSurfaceRoutes keeps route metadata deterministic for governance
// review and tests.
func sortConsumerSurfaceRoutes(routes []*ConsumerSurfaceRouteSnapshot) {
