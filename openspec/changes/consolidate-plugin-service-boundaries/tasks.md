...
- [ ] **FB-3**: 继续减少`apps/lina-core/internal/service/plugin`根目录小文件数量
...
- 阻断验证：`cd apps/lina-core && go test ./internal/service/plugin -count=1`、`cd apps/lina-core && go test ./internal/service/plugin/... -count=1`和`cd apps/lina-core && go test ./internal/service/plugin/internal/hostservices -count=1`当前失败于另一个活跃变更`auth-client-type-session-metadata`的未完成 DAO/Entity 同步：`internal/service/session/session_impl.go`已引用`do.SysOnlineSession.ClientType`、`dao.SysOnlineSession.Columns().ClientType`和`entity.SysOnlineSession.ClientType`，但生成代码尚未包含这些字段。因此 FB-3 暂不标记完成，待该外部编译阻断解决后需重跑插件根包和全量插件服务测试。
