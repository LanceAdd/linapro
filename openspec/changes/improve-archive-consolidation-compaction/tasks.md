...
- 修复：更新`.agents/skills/lina-archive-consolidate/SKILL.md`，在技能描述、读取步骤、高价值摘要压缩原则、`tasks.md`压缩模板和硬性规则中明确逐目录读取完整归档语义、禁止脚本压缩正文、`tasks.md`以减少存储空间为首要目标；同时修正单迭代分组规则，要求单个变更也必须语义重写和压缩，不得直接复制原文。同步更新`specs/archive-consolidation-compaction/spec.md`，补充可验证需求和场景。
...
- 验证：`rg -n "逐目录完整读取|禁止脚本压缩正文|减少存储空间|最短维护摘要|specs/\\*\\*/\\*\\.md|自动摘要程序|脚本输出|proposal\\.md.*design\\.md.*tasks\\.md.*specs/" .agents/skills/lina-archive-consolidate/SKILL.md openspec/changes/improve-archive-consolidation-compaction/specs/archive-consolidation-compaction/spec.md`通过，命中技能说明和增量规范中的新增门禁。
- 验证：`git diff --check -- .agents/skills/lina-archive-consolidate/SKILL.md openspec/changes/improve-archive-consolidation-compaction`通过，无输出。
