...
### Requirement: 插件清单必须校验结构化插件 ID

插件清单生命周期 SHALL 在加载源码插件 manifest、动态插件 artifact manifest 和插件依赖声明时统一校验结构化插件 ID。校验规则 MUST 复用插件 ID 治理能力定义的 `<author>-<domain>-<capability>`、domain 白名单、`core` 保留 capability 和 64 字符长度限制。任一插件 ID 校验失败时，系统 MUST 拒绝该 manifest 或 artifact，并返回可诊断错误。

#### Scenario: 源码插件 manifest ID 校验失败
- **WHEN** 源码插件 `plugin.yaml` 声明 `id: monitor-server`
- **THEN** 宿主拒绝加载该源码插件 manifest
- **AND** 错误说明插件 ID 必须使用结构化命名
...
- **AND** artifact 内嵌 manifest 声明 `id: plugin-demo-dynamic`
- **THEN** 宿主拒绝该 artifact
- **AND** 错误说明动态插件 ID 不符合结构化插件 ID 规则

#### Scenario: 插件依赖声明使用旧 ID
- **WHEN** 插件 manifest 的 `dependencies.plugins[].id` 声明 `plugin-demo-source`
- **THEN** 宿主拒绝该 manifest
- **AND** 错误说明插件依赖 ID 必须使用规范化插件 ID

### Requirement: 插件清单资源命名空间必须使用当前插件 ID

插件清单生命周期 SHALL 要求 manifest 声明的菜单、权限、运行时 i18n、apidoc i18n、cron 和动态前端资产入口使用当前规范化插件 ID 派生命名空间。宿主不得从旧 ID 自动推导或补齐这些资源。
